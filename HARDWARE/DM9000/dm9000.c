...
u8 DM9000_Init(void)
{
...
#else
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6; 		//PD6 推挽输出 
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;//推挽输出
	GPIO_Init(GPIOD,&GPIO_InitStructure);
#endif
	...
	sw_log("DM9000_Init", SW_LOG_LEVEL_DEBUG, "DM9000 ID:%#x\r\n",temp);
...
#if 0 // no need to check the speed, the camera will be ready later and the link will be set up.
...
/*
********************************************************************************************
* 函数名：DM9000_PHY_ReadReg
* 返回值：读取到的PHY寄存器值
* 参  数：reg:要读的PHY寄存器
* 描  述：读取DM9000的PHY的指定寄存器
*
*********************************************************************************************
*/
...
/*
********************************************************************************************
* 函数名：DM9000_PHY_WriteReg
* 返回值：无
* 参  数：reg:要写数据的PHY寄存器
					data:要写入的值
* 描  述：向DM9000的PHY寄存器写入指定值
*
*********************************************************************************************
*/
...
*********************************************************************************************
*/
void DM9000_Set_PhyPower(u8 mode)
{
	u16 BMCR_Value;	
	BMCR_Value = DM9000_PHY_ReadReg(0x0);
	if (mode == 1)
		BMCR_Value &= ~(1 << 11);
	else
		BMCR_Value |= (1 << 11);
...
int DM9000_Get_PhyPower(void)
{
	u16 BMCR_Value;	
	BMCR_Value = DM9000_PHY_ReadReg(0x0);
	if (((BMCR_Value >> 11) & 0x1)  == 0x1)
		return 0; //indicate it power down
	else
		return 1;
...
/*
********************************************************************************************
* 函数名：DM9000_Set_Multicast
* 返回值：无 
* 参  数：*multicastaddr:指向多播地址 
* 描  述：设置DM9000的组播地址
*
*********************************************************************************************
*/
...
void DM9000_SendPacket(struct pbuf *p)
{
	...
	while(((DM9000_ReadReg(DM9000_ISR)&0X02)==0) && (DM9000_Get_PhyPower() == 1));			//等待发送完成 
...
/*
********************************************************************************************
* 函数名：DM9000_Receive_Packet
* 返回值：*p:pbuf格式的接收到的数据包
*					接收到的数据包存放在DM9000的RX FIFO中，地址为0X0C00~0X3FFF，接收到的数据包的前四个字节并不是真实的数据，而是有特定含义的
