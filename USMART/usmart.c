...
{
	u8 sta,len;
	u8	USMART_Recv[256] ={0};
	
		if(g_usart1_receive_stat == 1)  //默认接受到数据完成
		{
			g_usart1_receive_stat = 0;
			DMA_usart1_Buf[g_usart1_recveive_cnt] = '\0';
			memcpy(USMART_Recv, DMA_usart1_Buf, g_usart1_recveive_cnt + 1);
			sta=usmart_dev.cmd_rec(USMART_Recv);//得到函数各个信息
			if(sta == 0)  usmart_dev.exe();
			else
			{
				len = usmart_sys_cmd_exe(USMART_Recv);
				...
				if(sta)
				{
					...
					case USMART_NOFUNCFIND:
						printf("No matching function was found!\r\n");   			
						break;	
					}
